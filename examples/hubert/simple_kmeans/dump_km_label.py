...
def get_feat_iterator(feat_dir, split, nshard, rank):
    ...
    file_list = [os.path.join(feat_dir, f) for f in os.listdir(feat_dir) if f.startswith(pattern)]
    ...
    def iterate():
        for filename in file_list:
            feat = np.load(filename, mmap_mode='r')
            yield feat
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lab_dir",  type=str, default="data/label")
